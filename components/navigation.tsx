...
export function Navigation({ 
  ...
  const renderNavLinks = (isMobile: boolean) => {
    ...
    const inactiveClass = isMobile ? "" : "text-slate-700 dark:text-slate-300 hover:text-blue-500 dark:hover:text-blue-400"
                  ...
                  {user && navItems === mainSiteNavItems ? (
                    <>
                      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.2 }}>
                        <Link href="/dashboard" onClick={() => setIsMobileMenuOpen(false)} className="nav-item block font-mono text-lg font-medium text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors py-2">
                          <span className={pathname === "/dashboard" ? "active" : ""}>Dashboard</span>
                        </Link>
                      </motion.div>
                      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.3 }}>
                        <Link href="/practice" onClick={() => setIsMobileMenuOpen(false)} className="nav-item block font-mono text-lg font-medium text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors py-2">
                          <span className={pathname.startsWith("/practice") ? "active" : ""}>Practice</span>
                        </Link>
                      </motion.div>
                      <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.4 }}>
                        <Link href="/results" onClick={() => setIsMobileMenuOpen(false)} className="nav-item block font-mono text-lg font-medium text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors py-2">
                          <span className={pathname.startsWith("/results") ? "active" : ""}>Results</span>
                        </Link>
                      </motion.div>
                      {user.email === "admin@celpius.ai" && (
                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: 0.6 }}>
                          <Link href="/admin" onClick={() => setIsMobileMenuOpen(false)} className="block font-mono text-lg font-medium text-slate-700 dark:text-slate-300 hover:text-blue-600 dark:hover:text-blue-400 transition-colors py-2">
                            Admin
                          </Link>
                        </motion.div>
                      )}
                    ...
                    <>
                      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.6 }}>
                        <Link href="/auth/login" onClick={() => setIsMobileMenuOpen(false)}>
                          <Button variant="ghost" className="w-full justify-start font-mono">Sign In</Button>
                        </Link>
                      </motion.div>
