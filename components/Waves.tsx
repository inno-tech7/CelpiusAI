...
const Waves: React.FC<WavesProps> = ({
  ...
  useEffect(() => {
    ...
    function drawLines() {
      ...
      const { lineColor, fadeTo, fadeAngle, fadeStart, fadeEnd } = configRef.current;
      const angleRad = (fadeAngle * Math.PI) / 180;
      const x = Math.cos(angleRad);
      const y = Math.sin(angleRad);
      const x1 = width / 2 - (x * width) / 2;
      const y1 = height / 2 - (y * height) / 2;
      const x2 = width / 2 + (x * width) / 2;
      const y2 = height / 2 + (y * height) / 2;

      const gradient = ctx.createLinearGradient(x1, y1, x2, y2);
      gradient.addColorStop(Math.max(0, Math.min(1, fadeStart)), lineColor);
      gradient.addColorStop(Math.max(0, Math.min(1, fadeEnd)), fadeTo);
