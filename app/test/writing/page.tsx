        ...
        >
          <Card className="border-slate-700/50 bg-slate-800/30 backdrop-blur-sm">
            <CardHeader className="text-center">
              <CardTitle className="text-3xl text-white">Writing Test Results</CardTitle>
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <Card className="border-slate-600/30 bg-slate-700/30">
                  <CardHeader>
                    <CardTitle className="text-lg text-white">Task 1: Email</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      <div className="text-2xl font-bold text-blue-400">{task1Eval.score}/6</div>
                      <div className="text-sm text-gray-400">
                        Word count: {countWords(responses[1] || '')} words
                      </div>
                ...
                <Card className="border-slate-600/30 bg-slate-700/30">
                  <CardHeader>
                    <CardTitle className="text-lg text-white">Task 2: Survey Response</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-2">
                      <div className="text-2xl font-bold text-blue-400">{task2Eval.score}/6</div>
                      <div className="text-sm text-gray-400">
                        Word count: {countWords(responses[2] || '')} words
                      </div>
                      <div className="space-y-1">
      ...
      <div className="card-outline font-sans text-white">
        <header className="flex items-center justify-between rounded-t-[23px] border-[1px] border-b-[#3b4687] bg-blue-950/0 p-4 max-435:flex-col max-435:items-start">
          <h1 className="max-w-[70%] font-mono text-lg font-semibold text-blue-400 max-435:pb-[2rem]">
            Practice Test A - Writing {currentTaskData.title}
          </h1>
          <div className="flex items-center space-x-4 max-435:space-x-32">
            <span className="text-sm text-slate-400">
              Time remaining:{' '}
              <span className="font-bold text-red-500">{formatTime(taskTimeRemaining)}</span>
            </span>
            <Button
        ...
        <main className="grid grid-cols-3 gap-6 max-1024:grid-cols-1">
          {/* Left Column */}
          <div className="col-span-1 flex flex-col border-r border-slate-700 p-4 sm:p-6 md:p-8 max-1024:border-r-0">
            <div className="flex-grow">
              ...
              <ScrollArea className="h-[calc(100vh-250px)] max-1024:h-[calc(35vh-250px)]">
                <div className="whitespace-pre-line rounded-md bg-slate-800/50 p-4 leading-relaxed text-slate-300">
          ...
          <div className="col-span-2 flex flex-col p-4 sm:p-6 md:p-8">
            <div className="flex-grow">
              <div className="mb-4 flex items-center rounded-md bg-blue-900/0 p-3">
                <AlertCircle className="mr-3 text-blue-400" />
                <p className="whitespace-pre-line font-mono font-semibold text-blue-400">
                  {currentTaskData.taskPrompt}
                </p>
              </div>
              <ul className="mb-4 list-outside list-disc space-y-2 pl-6 text-slate-300">
              ...
              <Textarea
                ...
                className="min-h-[300px] resize-none border-slate-700 bg-slate-800/50 text-gray-300"
              />
              <div className="mt-2 text-sm text-slate-400">Word Count: {wordCount}</div>
            </div>
          </div>
        </main>
