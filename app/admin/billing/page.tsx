    ...
    >
      <div>
        ...
        <p className="mt-2 text-muted-foreground">
          Manage subscription plans, pricing, and view transactions.
        </p>
      </div>
        ...
        <Card className="glassmorphic-dashboard">
          ...
          <CardContent className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {plans.map((plan) => (
              <div
                key={plan.name}
                className={`rounded-2xl border p-6 ${editingPlan === plan.name ? 'border-blue-300/70' : plan.isCurrent ? 'border-blue-500/50 bg-blue-900/20' : 'border-white/10 bg-black/20'} transition-all duration-300`}
              >
                {editingPlan === plan.name ? (
                  <div className="space-y-4">
                    <Input
                      ...
                      className="rounded-md border border-blue-400/50 bg-transparent p-2 text-xl font-semibold focus-visible:ring-1 focus-visible:ring-blue-300"
                    />
                    <div className="my-4 flex items-baseline gap-2">
                      <Input
                        ...
                        onChange={handleInputChange}
                        className="h-auto rounded-md border border-blue-400/50 bg-transparent p-2 text-3xl font-bold focus-visible:ring-1 focus-visible:ring-blue-300"
                      />
                      <Input
                        name="period"
                        value={editedData.period}
                        onChange={handleInputChange}
                        className="h-auto rounded-md border border-blue-400/50 bg-transparent p-2 text-lg focus-visible:ring-1 focus-visible:ring-blue-300"
                      />
                    </div>
                    <Textarea
                      name="features"
                      ...
                      onChange={handleInputChange}
                      className="rounded-md border border-blue-400/50 bg-transparent p-2 text-sm focus-visible:ring-1 focus-visible:ring-blue-300"
                      rows={5}
                    />
                  ...
                  <div>
                    <div className="flex items-start justify-between">
                      ...
                      <Button
                        ...
                        className="-mr-2 -mt-2 h-8 w-8 hover:bg-white/10"
                      >
                    ...
                    <ul className="space-y-2 text-sm text-muted-foreground">
                      {plan.features.map((feature) => (
                        <li key={feature} className="flex items-start">
                          <Check className="mr-2 mt-1 h-4 w-4 flex-shrink-0 text-green-400" />
                          <span>{feature}</span>
                        </li>
                      ))}
                    </ul>
                ...
                <TableBody>
                  {recentTransactions.map((txn) => (
                    <TableRow key={txn.id} className="border-white/10">
                      ...
                      <TableCell>
                        <Badge
                          variant={txn.status === 'Success' ? 'secondary' : 'destructive'}
